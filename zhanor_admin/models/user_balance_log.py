...
class UserBalanceLog(Base):
    ...
    user_id = Column(Integer, nullable=False,comment='Member ID') 
    balance = Column(Numeric, nullable=False,comment='Change in Balance') 
    before = Column(Numeric, nullable=False,comment='Balance Before Change') 
    after = Column(Numeric, nullable=False,comment='Balance After Change') 
    memo = Column(String(255),comment='Memo/Note') 
