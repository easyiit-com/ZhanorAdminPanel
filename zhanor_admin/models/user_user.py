...
import bcrypt
from sqlalchemy import Numeric,Date,Integer,String,SmallInteger,Column,DateTime
...
class UserUser(Base):
    ...
    user_group_id = Column(Integer, nullable=False,comment='Group ID') 
    name = Column(String(32), nullable=False,comment='Username') 
    nickname = Column(String(50), nullable=False,comment='Nickname') 
    password = Column(String(120), nullable=False,comment='Password') 
    email = Column(String(100), nullable=False,comment='Email') 
    mobile = Column(String(16), nullable=False,comment='Mobile Phone Number') 
    avatar = Column(String(255),comment='Avatar') 
    level = Column(SmallInteger, nullable=False, default = '0' ,comment='Level') 
    gender = Column(ENUM('female', 'male'), nullable=False, default = 'male' ,comment='Gender') 
    birthday = Column(Date,comment='Date of Birth') 
    bio = Column(String(100),comment='Motto') 
    balance = Column(Numeric, default = '0' ,comment='Balance') 
    score = Column(Integer, nullable=False, default = '0' ,comment='Points') 
    successions = Column(Integer, nullable=False, default = '0' ,comment='Consecutive Login Days') 
    maxsuccessions = Column(Integer, nullable=False, default = '0' ,comment='Maximum Consecutive Login Days') 
    prevtime = Column(DateTime,comment='Previous Login Time') 
    logintime = Column(DateTime,comment='Login Time') 
    loginip = Column(String(50),comment='Login IP Address') 
    loginfailure = Column(SmallInteger, nullable=False, default = '0' ,comment='Failed Login Attempts') 
    joinip = Column(String(50),comment='Joining IP Address') 
    createtime = Column(DateTime,comment='Creation Time') 
    updatetime = Column(DateTime,comment='Update Time') 
    verification = Column(String(255),comment='Verification') 
    token = Column(String(50),comment='Token') 
    status = Column(ENUM('normal', 'hidden'), default = 'normal' ,comment='Status') 
