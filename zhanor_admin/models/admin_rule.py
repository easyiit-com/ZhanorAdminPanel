...
from sqlalchemy import Column, ForeignKey,String,SmallInteger,Integer,DateTime
...
class AdminRule(Base):
    ...
    type = Column(ENUM('menu', 'action'), nullable=False, default = 'menu' ,comment='Type') 
    pid = Column(Integer, nullable=False, default = '0' ,comment='PARENT ID') 
    # pid = Column(Integer, ForeignKey('admin_rule.id'), nullable=False, default=0, comment='PARENT ID')
    addon = Column(SmallInteger, nullable=False,default = '0', comment='From Addon') 
    name = Column(String(150), nullable=False,comment='Route Name') 
    url_path = Column(String(50), nullable=False,comment='Url_Path') 
    title = Column(String(50), nullable=False,comment='Title') 
    description = Column(String(500), default = '' ,comment='Description') 
    icon = Column(String(50), default = '' ,comment='Icon') 
    menutype = Column(ENUM('addtabs', 'blank', 'dialog', 'ajax'), default = 'blank' ,comment='Menutype') 
    extend = Column(String(255), default = '' ,comment='Extend') 
    createtime = Column(DateTime,comment='Createtime') 
    updatetime = Column(DateTime,comment='Updatetime') 
    weigh = Column(Integer, nullable=False, default = '0' ,comment='Weigh') 
    status = Column(ENUM('normal', 'hidden'), nullable=False, default = 'normal' ,comment='Status') 
 
    # 
    # children = relationship("AdminRule", backref=backref("parent", remote_side=[id]), cascade="all, delete-orphan")
