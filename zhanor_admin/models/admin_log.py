...
from sqlalchemy import DateTime,Text,String,Column,Integer
...
class AdminLog(Base):
    ...
    admin_id = Column(Integer, nullable=False,comment='Administrator ID') 
    username = Column(String(30), nullable=False,comment='Administrator Name') 
    url = Column(String(1500), nullable=False,comment='Operated Page') 
    title = Column(String(100),comment='Log Title') 
    content = Column(Text, nullable=False,comment='Content') 
    ip = Column(String(50), nullable=False,comment='IP Address') 
    useragent = Column(String(255),comment='User Agent') 
    createtime = Column(DateTime,comment='Operation Time') 
