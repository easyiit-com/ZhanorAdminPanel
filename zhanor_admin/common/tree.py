...
class Tree:
        ...
        if childs:
            for value in childs.values():
                ...
                value.update({
                    ...
                    "@class": (f" active" if value['selected'] else '') + (f" disabled" if value['disabled'] else '') + (f" treeview{' treeview-open' if config('fastadmin.show_submenu') else ''}" if childdata else '')
                })
    ...
    def getTreeList(self, data=None, field='name'):
        ...
        for value in data:
            ...
            del value['childlist']
            value[field] = value['spacer'] + ' ' + _(value[field])
